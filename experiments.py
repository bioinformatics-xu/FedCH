...
def get_args():
    ...
    parser.add_argument('--dataset', type=str, default='CoronaryHeartDisease', help='dataset used for training，coronary10/coronary1/coronary2/coronary3/heart/kaggle/CoronaryHeartDisease/HeartAttackDataset/HeartDiseaseDataset')
    parser.add_argument('--net_config', type=lambda x: list(map(int, x.split(', '))))
    parser.add_argument('--partition', type=str, default='noniid-labeldir2', help='alg/iid-labeldir/noniid-labeldir2')
    ...
    parser.add_argument('--alg', type=str, default='scaffold',help='fl algorithms: fedavg/fedprox/scaffold/fednova/Nfednova/moon')
    ...
    parser.add_argument('--datadir', type=str, required=False, default="./data/CoronaryHeartDisease", help="Data directory")
    ...
    parser.add_argument('--rho', type=float, default=0.8, help='Parameter controlling the momentum SGD')
    parser.add_argument('--sample', type=float, default=1, help='Sample ratio for each communication round')
    parser.add_argument('--alpha', type=float, default=0.99, help='Historical data weight')
            ...
            for net_i in range(n_parties):
                if args.dataset == "generated":
                    ...
                    if args.dataset == 'covtype':
                        ...
                        output_size = 2
                        hidden_sizes = [16, 8]
                    elif args.datadir == './data/coronary123':
                        input_size = 42
                        output_size = 3
                        hidden_sizes = [16, 8]
                        ...
                        output_size = 2
                        hidden_sizes = [16,8]
                    elif args.datadir == './data/CoronaryHeartDisease':
                        input_size = 9
                        output_size = 2
                        hidden_sizes = [16,8]
                    elif args.datadir == './data/HeartAttackDataset':
                        input_size = 13
                        output_size = 2
                        hidden_sizes = [16,8]
                    elif args.datadir == './data/HeartDiseaseDataset':
                        input_size = 13
                        output_size = 2
                        hidden_sizes = [16,8]
...
def train_net_fedsss(net_id, net, global_net, train_dataloader, test_dataloader, epochs, lr, args_optimizer, mu, device="cpu"):
    logger.info('Training network %s' % str(net_id))
    logger.info('n_training: %d' % len(train_dataloader))
    logger.info('n_test: %d' % len(test_dataloader))
...
def train_net_Nfednova(net_id, net, global_net , train_dataloader, test_dataloader, epochs, lr, mu , args_optimizer, device="cpu"):
    logger.info('Training network %s' % str(net_id))
...
def train_net_moon(net_id, net, global_net, previous_nets, train_dataloader, test_dataloader, epochs, lr, args_optimizer, mu, temperature, args,
    ...
    train_acc, train_auc ,train_f1 = compute_accuracy(net, train_dataloader, moon_model=True, device=device)
    test_acc, conf_matrix, test_auc ,test_f1 = compute_accuracy(net, test_dataloader, get_confusion_matrix=True, moon_model=True, device=device)
    ...
    if args.loss != 'l2norm':
        for previous_net in previous_nets:
            previous_net.to('cpu')
    train_acc, auc ,f1 = compute_accuracy(net, train_dataloader, moon_model=True, device=device)
    test_acc, conf_matrix, auc ,f1 = compute_accuracy(net, test_dataloader, get_confusion_matrix=True, moon_model=True, device=device)
...
def local_train_net_fedsss(nets, selected, global_model, args, net_dataidx_map, test_dl = None, device="cpu"):
    avg_acc = 0.0
    acc=[]
    for net_id, net in nets.items():
        if net_id not in selected:
        ...
        trainacc, testacc = train_net_fedsss(net_id, net, global_model, train_dl_local, test_dl, n_epoch, args.lr, args.optimizer, args.mu, device=device)
...
def local_train_net_Nfednova(nets, selected, global_model, args, net_dataidx_map, test_dl = None, device="cpu"):
    avg_acc = 0.0
    ...
    for net_id, net in nets.items():
        ...
        trainacc, testacc, a_i, d_i = train_net_Nfednova(net_id, net, global_model, train_dl_local, test_dl, n_epoch, args.lr, args.mu , args.optimizer, device=device)
...
def ou_distance(a, b):
    diff = a - b  # 计算差值
    squared_diff = np.square(diff)  # 对差值进行平方
    squared_difff = np.array([])
    squared_difff=np.append(squared_diff,squared_difff)
    sum_squared_diff = np.sum(squared_difff)  # 对平方差值进行求和
    distance = np.sqrt(sum_squared_diff)  # 开方得到欧氏距离
    return distance
"""
#计算余弦相似度cos
...
def cosine_similarity(dict1, dict2):
    dict11 = np.array([])
    vec1 = np.append(dict1, dict11)
    dict22 = np.array([])
    vec2 = np.append(dict2, dict22)

    numerator = sum([vec1[i] * vec2[i] for i in range(min(len(vec1), len(vec2)))])

    norm_vec1 = math.sqrt(sum(val ** 2 for val in vec1))
    norm_vec2 = math.sqrt(sum(val ** 2 for val in vec2))
...
if __name__ == '__main__':
    ...
    if args.alg == 'fedavg':
        ...
        nets, local_model_meta_data, layer_type = init_nets(args.net_config, args.dropout_p, args.n_parties, args)
        global_models, global_model_meta_data, global_layer_type = init_nets(args.net_config, 0, 1, args)
        global_model = global_models[0]          #初始化全局模型

        global_para = global_model.state_dict()      #全局模型的参数保存到global_para中

        if args.is_same_initial:
            for net_id, net in nets.items():
                net.load_state_dict(global_para)       #全局参数加载到每个本地net中
        ...
        for round in range(args.comm_round):
            ...
            global_para = global_model.state_dict()                 #将全局模型的参数再次保存到global_para中
            if round == 0:
                if args.is_same_initial:
                    for idx in selected:
                        nets[idx].load_state_dict(global_para)
            ...
            aacc=local_train_net_fedsss(nets, selected, global_model, args, net_dataidx_map, test_dl = test_dl_global, device=device)
            for idx in range(len(selected)):
                acc.append(1-aacc[idx])
            ...
            # 计算 acc_weight
            for idx in range(len(selected)):
                acc_weight.append(acc[idx]/total_acc)
                print('client' + str(idx) + 'acc_weight:' + str(acc_weight[idx]))
            ...
            for idx in range(len(selected)):
                ...
                for key in net_para:
                    distance = ou_distance(net_para[key],global_para[key])
                    sum_distance=sum_distance+distance            # sum_distance是client[idx] model和global model的欧氏距离
                ou.append(sum_distance)

            total_distance=0
            total_distance=sum(ou)              #求总的欧氏距离
"""
            ...
            for idx in range(len(selected)):
                ...
                for key in net_para:
                    cosine = cosine_similarity(net_para[key], global_para[key])
                    sum_cosine = sum_cosine + cosine  # sum_distance是client[idx] model和global model的欧氏距离
                cos.append(sum_cosine)

            total_cosine = 0
            total_cosine = sum(cos)

            #计算 similarity_weight
            for idx in range(len(selected)):
                similarity_weight.append(cos[idx] / total_cosine)
                print('client' + str(idx) + 'similarity_weight:' + str(similarity_weight[idx]))


                #distance=manhattan_distance(net_para, global_para)
                #print(distance)

                """”
                #calculate 余弦相似度
                #cos[idx]=cosine_similarity(ne_para[key], global_para[key])
                cos.append(cosine_similarity(ne_para, glob_para))
                sums=sums+cos[idx]

            # normalized  similarity（归一化后的相似度）
            for idx in range(len(selected)):
                similarity.append(cos[idx] / sums)
                print('client' + str(idx)+ 'similarity:'+str(similarity[idx]))
"""

            # 计算 total_weight
            for idx in range(len(selected)):
                client_weight.append(acc_weight[idx] + similarity_weight[idx])          #对应client的acc和similarity权重加到一起
            sum_client_weight=sum(client_weight)       #所有权重总值

            for idx in range(len(selected)):                                                  #对应client权重归一化
                total_weight.append(client_weight[idx]/sum_client_weight)
                print('client' + str(idx) + 'total_weight:' + str(total_weight[idx]))

            # Global Model Aggregation
            for idx in range(len(selected)):
                net_para = nets[selected[idx]].cpu().state_dict()
                last_global_para = global_model.state_dict()

                #print(net_para)
                if idx == 0:  ###计算全局模型参数global_para[key]；net_para[key]是本地模型参数；fed_avg_freqs[idx]是权重
                    for key in net_para:
                        #print(net_para[key])
                        global_para[key] = args.alpha * net_para[key] * total_weight[idx] + (1 - args.alpha) * last_global_para[key]
                else:
                    for key in net_para:
                        #print(net_para[key])
                        global_para[key] += args.alpha * net_para[key] * total_weight[idx] + (1 - args.alpha) * last_global_para[key]
...
"""
        ...
        df = pd.DataFrame({'acc_results': acc_results, 'auc_results': auc_results, 'f1_results': f1_results})
        # 将DataFrame写入Excel文件
        df.to_excel('fedsss.xlsx', index=False)

        # 创建一个DataFrame对象
        df = pd.DataFrame({' ppv_results': ppv_results, 'npv_results': npv_results, 'spe_results': spe_results,
                           'sen_results': sen_results})
        # 将DataFrame写入Excel文件
        df.to_excel('fedsss_ppv.xlsx', index=False)
        ...
        # 绘制AUC值的图表
        plt.plot(x, acc_results, label='Accuracy', c="green", alpha=0.6, marker='.', lw=1)
        plt.plot(x, auc_results, label='AUC', c="orange", alpha=0.6, marker='.', lw=1)
        plt.plot(x, f1_results, label='f1', c="blue", alpha=0.6, marker='.', lw=1)
        # 设置图例
        plt.legend()
        # 设置x轴和y轴标签
        plt.xlabel('Epochs')
        ...
        df = pd.DataFrame({'acc_results': acc_results, 'auc_results': auc_results, 'f1_results': f1_results})
        # 将DataFrame写入Excel文件
        df.to_excel('scaffold.xlsx', index=False)

        # 创建一个DataFrame对象
        df = pd.DataFrame({' ppv_results': ppv_results, 'npv_results': npv_results, 'spe_results': spe_results,
                           'sen_results': sen_results})
        # 将DataFrame写入Excel文件
        df.to_excel('scaffold_ppv.xlsx', index=False)
        ...
        plt.xlabel('Epochs')
        plt.ylabel('Score')
        plt.title('scaffold Model Score')
        # 显示图形
        plt.show()
        ...
        for round in range(args.comm_round):
            ...
            _, a_list, d_list, n_list = local_train_net_Nfednova(nets, selected, global_model, args, net_dataidx_map,
        ...
        for round in range(args.comm_round):
            ...
            train_acc, auc ,f1 = compute_accuracy(global_model, train_dl_global, moon_model=True)
            test_acc, conf_matrix, auc ,f1 = compute_accuracy(global_model, test_dl_global, get_confusion_matrix=True, moon_model=True)
