...
from model import *
from datasets import MNIST_truncated, CIFAR10_truncated, CIFAR100_truncated, ImageFolder_custom, SVHN_custom, FashionMNIST_truncated, CustomTensorDataset, CelebA_custom, FEMNIST, Generated, genData, kaggle, heart ,coronary123, coronary1, coronary2, coronary10, HeartDiseaseDataset, CoronaryHeartDisease, HeartAttackDataset
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4):
    ...
    if dataset == 'mnist':
    ...
    elif datadir == "./data/kaggle":
        print("")
        cell_features = pd.read_csv(datadir + '/cardio_feature.csv')
        x_origin = cell_features.iloc[:, 2:]
        ...
        cardio_class = pd.read_csv(datadir + '/cardio_class.csv')
    ...
    elif datadir == "./data/heart":
        print("")
        cell_features = pd.read_csv(datadir + '/heart_feature.csv')
        ...
        # DataFrame的单元格和excel很像，可以设置列名columns与行名index
        x_origin = x_origin[x_origin.columns.sort_values()]
        cardio_class = pd.read_csv(datadir + '/heart_lable.csv')
        y_origin = cardio_class['heart']
        ...
        np.save(datadir + "/y_train.npy", y_train)
        np.save(datadir + "/y_test.npy", y_test)
    elif datadir == "./data/coronary123":
        print("")
        cell_features = pd.read_csv(datadir + '/coronary123_feature.csv')
        x_origin = cell_features.iloc[:, 0:]
        #在数据表中提取出相应的特征数据，第一个：获取所有行，选择所有行和列，但排除第 1列（前边代表行后边代表列）
        scaler = preprocessing.MinMaxScaler() #标准化数据——fit，fit_transform，transform
        x_features = scaler.fit_transform(x_origin)
        ...
        #DataFrame的单元格和excel很像，可以设置列名columns与行名index
        x_origin = x_origin[x_origin.columns.sort_values()]
        cardio_class = pd.read_csv(datadir + '/coronary123_lable.csv')
    ...
    elif datadir == "./data/coronary1":
        print("")
        cell_features = pd.read_csv(datadir + '/coronary1_feature.csv')
        x_origin = cell_features.iloc[:, 0:]
        #在数据表中提取出相应的特征数据，第一个：获取所有行，选择所有行和列，但排除第 1列（前边代表行后边代表列）
        scaler = preprocessing.MinMaxScaler() #标准化数据——fit，fit_transform，transform
        x_features = scaler.fit_transform(x_origin)
        ...
        #DataFrame的单元格和excel很像，可以设置列名columns与行名index
        x_origin = x_origin[x_origin.columns.sort_values()]
        cardio_class = pd.read_csv(datadir + '/coronary1_lable.csv')
        y_origin = cardio_class['Coronary+HighBlood']
    ...
    elif datadir == "./data/coronary2":
        print("")
        cell_features = pd.read_csv(datadir + '/coronary2_feature.csv')
        x_origin = cell_features.iloc[:, 0:]
        #在数据表中提取出相应的特征数据，第一个：获取所有行，选择所有行和列，但排除第 1列（前边代表行后边代表列）
        scaler = preprocessing.MinMaxScaler() #标准化数据——fit，fit_transform，transform
        x_features = scaler.fit_transform(x_origin)
        ...
        #DataFrame的单元格和excel很像，可以设置列名columns与行名index
        x_origin = x_origin[x_origin.columns.sort_values()]
        cardio_class = pd.read_csv(datadir + '/coronary2_lable.csv')
        y_origin = cardio_class['coronary+highblood']
    ...
    elif datadir == "./data/CoronaryHeartDisease":
        print("")
        cell_features = pd.read_csv(datadir + '/CHD_feature.csv')
        x_origin = cell_features.iloc[:, 0:]
        #在数据表中提取出相应的特征数据，第一个：获取所有行，选择所有行和列，但排除第 1列（前边代表行后边代表列）
        scaler = preprocessing.MinMaxScaler() #标准化数据——fit，fit_transform，transform
        x_features = scaler.fit_transform(x_origin)
        ...
        #DataFrame的单元格和excel很像，可以设置列名columns与行名index
        x_origin = x_origin[x_origin.columns.sort_values()]
        cardio_class = pd.read_csv(datadir + '/CHD_lable.csv')
        y_origin = cardio_class['lable']

        X_train, X_test, y_train, y_test = train_test_split(x_origin, y_origin, test_size=0.2, random_state=66)
        X_train = np.array(X_train, dtype='float32')
        X_test = np.array(X_test, dtype='float32')
        y_train = np.array(y_train, dtype='int64')
        y_test = np.array(y_test, dtype='int64')
    ...
    elif datadir == "./data/HeartAttackDataset":
        print("")
        cell_features = pd.read_csv(datadir + '/heart attack_feature.csv')
        x_origin = cell_features.iloc[:, 0:]
        #在数据表中提取出相应的特征数据，第一个：获取所有行，选择所有行和列，但排除第 1列（前边代表行后边代表列）
        scaler = preprocessing.MinMaxScaler() #标准化数据——fit，fit_transform，transform
        x_features = scaler.fit_transform(x_origin)
        ...
        #DataFrame的单元格和excel很像，可以设置列名columns与行名index
        x_origin = x_origin[x_origin.columns.sort_values()]
        cardio_class = pd.read_csv(datadir + '/heart attack_lable.csv')
        y_origin = cardio_class['lable']
    ...
    elif datadir == "./data/HeartDiseaseDataset":
        print("")
        cell_features = pd.read_csv(datadir + '/heart disease_feature.csv')
        x_origin = cell_features.iloc[:, 0:]
        #在数据表中提取出相应的特征数据，第一个：获取所有行，选择所有行和列，但排除第 1列（前边代表行后边代表列）
        scaler = preprocessing.MinMaxScaler() #标准化数据——fit，fit_transform，transform
        x_features = scaler.fit_transform(x_origin)
        ...
        #DataFrame的单元格和excel很像，可以设置列名columns与行名index
        x_origin = x_origin[x_origin.columns.sort_values()]
        cardio_class = pd.read_csv(datadir + '/heart disease_lable.csv')
        y_origin = cardio_class['lable']
    ...
    if partition == "homo":
        ...
        while min_size < min_require_size:
            ...
            #     # proportions_test = np.array([p_test * (len(idx_j_test) < N_test / n_parties) for p_test, idx_j_test in zip(proportions_test, idx_batch_test)])
            ...
            #     idx_batch_test = [idx_j_test + idx_test.tolist() for idx_j_test, idx_test in zip(idx_batch_test, np.split(idx_k_test, proportions_test))]
            ...
            proportions = np.random.dirichlet(np.repeat(beta, n_parties))
            proportions_test = proportions
            proportions_0 = (np.cumsum(proportions) * len(idx_k_0)).astype(int)[:-1]
            proportions_test_0 = (np.cumsum(proportions_test) * len(idx_k_test_0)).astype(int)[:-1]
            proportions_1 = (np.cumsum(proportions) * len(idx_k_1)).astype(int)[:-1]
            proportions_test_1 = (np.cumsum(proportions_test) * len(idx_k_test_1)).astype(int)[:-1]
    ...
    elif partition == "noniid-labeldir2":

        min_size = 0
        min_require_size = 10
        K = 10
        if dataset in ('celeba', 'covtype', 'a9a', 'rcv1', 'SUSY', 'heart', 'kaggle','coronary1'):
            K = 2
        N = y_train.shape[0]
        net_dataidx_map = {}
        while min_size < min_require_size:
            ...
            if n_parties == 6:
                proportions_0 = [0.3, 0.1, 0.1, 0.2, 0.2, 0.1]
                proportions_1 = [0.1, 0.3, 0.2, 0.1, 0.1, 0.1]

            if n_parties == 7:
                proportions_0 =[0.35, 0.0875, 0.0875, 0.175, 0.175, 0.0875, 0.0875]
                proportions_1 = [[0.125, 0.1875, 0.09375, 0.03125, 0.1875, 0.28125, 0.09375]]
            elif n_parties == 2:
                proportions_0 = [0.35, 0.65]
                proportions_1 = [0.7, 0.3]
            elif n_parties == 3:
                proportions_0 = [0.35, 0.25, 0.4]
                proportions_1 = [0.3, 0.5, 0.2]
            elif n_parties == 5:
                proportions_0 = [0.4, 0.1, 0.1, 0.2, 0.2]
                proportions_1 = [0.2, 0.3, 0.15, 0.05, 0.3]
            elif n_parties == 10:
                proportions_0 = [0.08, 0.15, 0.22, 0.14, 0.09, 0.06, 0.12, 0.07, 0.04, 0.03]
                proportions_1 = [0.16, 0.23, 0.07, 0.12, 0.09, 0.05, 0.10, 0.11, 0.05, 0.02]
                #proportions_0 = [0.1, 0.3, 0.02, 0.1, 0.08, 0.24, 0.01, 0.05, 0.06, 0.04]
                #proportions_1 = [0.2, 0.05, 0.08, 0.01, 0.03, 0.1, 0.15, 0.2, 0.05, 0.13]
    ...
    elif partition == "noniid-labeldir3":
        ...
        while min_size < min_require_size:
            ...
            idx_k_1 = np.where(y_train == 1)[0]
            np.random.shuffle(idx_k_1)
            idx_k_2 = np.where(y_train == 2)[0]
            np.random.shuffle(idx_k_2)
            ...
            if n_parties == 6:
                ...
                proportions_0 = [0.35, 0.65]
                proportions_1 = [0.7, 0.3]
                proportions_2 = [0.6, 0.4]
            elif n_parties == 5:
                proportions_0 = [0.4, 0.1, 0.1, 0.2, 0.2]
                proportions_1 = [0.2, 0.3, 0.15, 0.05, 0.3]
                proportions_2 = [0.2, 0.3, 0.15, 0.05, 0.3]
            elif n_parties == 10:
                proportions_0 = [0.08, 0.15, 0.22, 0.14, 0.09, 0.06, 0.12, 0.07, 0.04, 0.03]
                proportions_1 = [0.16, 0.23, 0.07, 0.12, 0.09, 0.05, 0.10, 0.11, 0.05, 0.02]
                # proportions_0 = [0.1, 0.3, 0.02, 0.1, 0.08, 0.24, 0.01, 0.05, 0.06, 0.04]
            ...
            proportions_0 = (np.cumsum(proportions_0) * len(idx_k_0)).astype(int)[:-1]
            proportions_1 = (np.cumsum(proportions_1) * len(idx_k_1)).astype(int)[:-1]
            proportions_2 = (np.cumsum(proportions_2) * len(idx_k_2)).astype(int)[:-1]
            idx_batch = [idx_j + idx.tolist() for idx_j, idx in zip(idx_batch, np.split(idx_k_0, proportions_0))]
            idx_batch = [idx_j + idx.tolist() for idx_j, idx in zip(idx_batch, np.split(idx_k_1, proportions_1))]
            idx_batch = [idx_j + idx.tolist() for idx_j, idx in zip(idx_batch, np.split(idx_k_2, proportions_2))]
            min_size = min([len(idx_j) for idx_j in idx_batch])
            for j in range(n_parties):
                np.random.shuffle(idx_batch[j])
                net_dataidx_map[j] = idx_batch[j]
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0, net_id=None, total=0):
    ...
    elif dataset == 'coronary123':
        dl_obj = coronary123
        transform_train = None
        transform_test = None

    elif dataset == 'coronary1' :
        dl_obj = coronary1
        transform_train = None
        transform_test = None

    elif dataset == 'coronary2' :
        dl_obj = coronary2
        transform_train = None
        transform_test = None

    elif dataset == 'coronary10' :
        dl_obj = coronary10
        transform_train = None
        transform_test = None

    elif dataset == 'CoronaryHeartDisease' :
        dl_obj = CoronaryHeartDisease
        transform_train = None
        transform_test = None

    elif dataset == 'HeartAttackDataset' :
        dl_obj = HeartAttackDataset
        transform_train = None
        transform_test = None

    elif dataset == 'HeartDiseaseDataset' :
        dl_obj = HeartDiseaseDataset
        transform_train = None
        transform_test = None
    ...
    elif dataset in ('mnist', 'femnist', 'fmnist', 'svhn', 'cifar10', 'cifar100', 'generated'):
